...
- Day 60: 你自己查得到的信号, 胜过任何全国排行榜 — 为什么全国性排名对个人决策几乎无用(官方十年职业预测在粗分层面有真实信息、在细分职业层面判「是否快于经济平均」只有 57%、基线 50%, 而个人决策恰恰发生在细分层面); 把「查排行榜」换成「查与你的决定直接对应的一手信号」: 本地供给量、逐年录取/竞争位次、官方预警清单、目标机构自己发布的横向数据; 一份「看起来像信号但不是信号」的清单(单年排名、含升学的达成率、判据不公开的红绿榜、年际波动大于趋势的一阶导); 通用方法: 先问这个指标的分母是不是你, 再问它的更新频率能不能覆盖你的决策周期(钩子·源自 deep-research #16)
