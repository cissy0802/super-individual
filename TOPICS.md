...
- Day 58: 被 AI 产出淹没时的举证责任设计 — 维护者/评审/安全团队面对 AI slop 洪水的工程对策: 把举证责任推回提交侧(只收带机器可验证产物的输入: 可复现用例 + 过测试的补丁), curl 两幕剧(<5%→反超 15-16%)与 kernel 新规(Torvalds "create a patch too"), 激励结构如何调制洪水而非恒久定律, 个人版"只收带 oracle 的输入"清单(钩子·源自 deep-research #5;编号避开已发布 day56/57, 可能被自动 routine 顺延)
