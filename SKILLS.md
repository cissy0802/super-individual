...
2. **数字必须挂来源和日期**：星数、安装量、排名一律写成「据 <来源>，<年-月>」，因为这些天天在变。**日期写来源页自己的发布/更新日期，不是你写稿那天**——一篇 2026-03 的榜单在 2026-08 引用，要写 2026-03（差得多的时候顺手去原站核一下现值，两个数都给）。
3. **不做软文**：每一期都要有诚实的代价 —— 这东西什么时候是负担、什么时候不如不装。
4. **工具打底，判据收尾**（本系列的重心，别写反）：正文必须**点名 6 个以上真实可装/可读的具体条目**（skill / plugin / MCP server / 官方文档），每个都给链接，并说清三件事——**它替你干掉哪件事**、**怎么装怎么用**（官方给了命令就抄命令，没有就写清放哪个目录）、**什么时候不该装**。判据放在**后半程**，作用是在这几个已经点名的候选之间做取舍。
   **跑偏的自检**：点名不足 6 个 → 跑偏；连着两节没出现任何一个具体条目名 → 跑偏；选购卡写成「装一个方法论型 + 一个单点型」这种占位而不是具体名字 → 跑偏。「判据可复用、工具会过时」是对的，但读者是来拿清单的，判据是清单的附加值，不是替代品。
5. 编号 `Skill N` 与文件名 `{slug}-skill{N}.html` 一一对应，按写作顺序递增。

**每期骨架**

`[榜单]` 期：**01 = 选品表**（6–10 个真实条目，每条固定「替你干掉什么 · 一句话理由 · 什么时候别装 · 链接」，先给表再讲道理）→ 02 = 这些东西为什么不能混着比 → 03 = 打开 SKILL.md 三分钟验货的判据 → 04 = 诚实的代价 → 选购卡点名「先装这三个」。

`[深挖]` 期：**01 = 它替你干掉什么 + 安装命令 + 第一条立刻能跑的用法** → 02 = 机制（它凭什么有效） → 03 = 真实用法与边界 → 04 = 代价与替代品（不装它你还能怎么办）。
...
- Skill 3 [深挖]: code-review-skills — 代码审查类 skills — 主角：`code-reviewer`（Agensi 装机榜第一）与官方 `webapp-testing`；审查清单进 prompt vs 进 skill；为什么「审查」是 skill 的甜区（高频 + 有客观 oracle）；假阳性治理；和 CI 门禁怎么分工
- Skill 4 [深挖]: git-workflow-skills — Git 工作流类 skills — 主角：`git-commit-writer` / `pr-description-writer` / `changelog-generator` 三件套（逐个给装法与实际输出）；diff 作为唯一输入的可靠性；团队约定如何编码成 examples pattern；什么时候一条 git hook 比 skill 更合适

## C2 · 元技能：做 skill 的 skill（Meta — Building Skills）

- Skill 5 [榜单]: meta-skills-roundup — 元技能榜单 — `skill-creator` / `mcp-builder` / `web-artifacts-builder` / `claude-api` 这类「造工具的工具」（注意真名是 web-artifacts-builder，不是 artifacts-builder）；官方 `spec/` 与 `template/`；为什么先写 eval 再写文档
...
- Skill 15 [深挖]: brand-guidelines — 品牌与风格类 skill — 主角：官方 `brand-guidelines` 与 `theme-factory`；把风格指南变成可执行约束；风格类 skill 最难的是验收标准

## C5 · 写作与内容（Writing & Content）

- Skill 16 [榜单]: writing-skills-roundup — 写作类 skills 榜单 — 至少覆盖 `humanize-writing`、`seo-optimizer`、官方 `doc-coauthoring` / `internal-comms`；去 AI 味 / 长文起草 / 摘要 / SEO 各解决什么；为什么写作类 skill 的口碑分歧最大
- Skill 17 [深挖]: humanizing-writing — 「去 AI 味」类 skill — 主角：`humanize-writing`（Agensi 装机榜前三）；它到底在删什么句式；把个人 voice 编成 skill 的做法；检测器军备竞赛的坑
- Skill 18 [深挖]: examples-over-rules — 写作类 skill 的通用配方：examples pattern — 拆官方 `brand-guidelines` / `doc-coauthoring` 里的样例段当范本；三对输入/输出比十条规则更管用；样例怎么选、几个够
...
- Skill 21 [深挖]: source-grounding — 来源接地与引用可信度 — 主角：`truth-first` 与官方 `discernment-nudge`；哪些内容模型几乎必编（URL/引文/日期）；工具接地怎么改判；二手 SEO 站不能当来源

## C7 · 数据与分析（Data & Analysis）

- Skill 22 [榜单]: data-skills-roundup — 数据分析类 skills 榜单 — NL→SQL / 图表 / 异常检测 / 报告生成；为什么这一类最依赖组织私有上下文
- Skill 23 [深挖]: domain-reference-skill — 按域拆 reference 的 skill 结构 — 以官方 `claude-api` skill 为拆解范本（一个 SKILL.md 挂多份领域文档，用到哪份读哪份）；schema 与口径规则写在哪
- Skill 24 [深挖]: plan-validate-execute — plan-validate-execute 模式 — 以官方 `docx` / `xlsx` 里的校验脚本为实例；让模型先产出可机器校验的中间计划，再执行；批量与破坏性操作的标准打法
...
- Skill 31 [深挖]: truth-first-skills — 「诚实优先」类 skill — 主角：`truth-first`（Agensi）；逼模型说「不知道」、逼它跑完代码再声称完成；把不确定性写进输出契约；怎么验收这类 skill
